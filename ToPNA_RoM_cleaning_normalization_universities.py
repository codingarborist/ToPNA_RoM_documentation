...
working_file = "ToPNA_RoM_pre_cleaning.csv"

rom_df = pd.read_csv(working_file)


rom_df['advisor']=rom_df['advisor'].str.replace("0", "O")
rom_df['advisor']=rom_df['advisor'].str.replace(r"\bO'", "\bO-")
...
rom_df['university']=rom_df['university'].str.replace(r"\*\b", " ")
rom_df['university']=rom_df['university'].str.replace(r"\*\B", "")


#len(rom_df['university'].unique()) == 365


rom_df['university_stripped']=rom_df['university']

chars_to_be_removed_from_unis = [r'\s\B', r'\bThe\s', r'\bthe\s', ',', 'of ', r'\)', "'"]
chars_to_replace_with_blank_s =['-', r'\sat\s', r'\(']


for char in chars_to_be_removed_from_unis:
    rom_df['university_stripped'] = rom_df['university_stripped'].str.replace(char, '')
    
for char in chars_to_replace_with_blank_s:
    rom_df['university_stripped'] = rom_df['university_stripped'].str.replace(char, ' ')

# len(rom_df['university_stripped'].unique()) == 223
...
rom_df['normalized_university']= rom_df['university_stripped'].replace(direct_mapping_universities_spelling)


#len(rom_df['normalized_university'].unique()) == 141 == len(inverted_mapping_universities_spelling)


rom_df.to_csv('ToPNA_RoM_working_manual_checking.csv')
